...
def _plot_metric_axis(ax, labels, values, style):
    ...
    if any(not _is_nan(v) for v in values):
        ...
        ax.text(
            0.5, 0.5, style.no_data_message, ha='center', va='center', transform=ax.transAxes
        )
        ax.set_axis_off()
    ax.legend()
...
_POSSESSION_STYLE = _MetricPlotStyle(
    marker='o',
    color='green',
    label='Avg Possession %',
    title='Average Possession % per Round',
    ylabel='Possession (%)',
    no_data_message='Possession data not available',
)

_SHOT_ACCURACY_STYLE = _MetricPlotStyle(
    marker='s',
    color='darkorange',
    label='Avg Shot Accuracy %',
    title='Average Shot Accuracy % per Round',
    ylabel='Shot Accuracy (%)',
    no_data_message='Shot accuracy data not available',
)
...
class TeamAnalyzer:
    ...
    def plot_performance_trends(self, team_name, seasons=default_seasons, logo_url=None):
        ...
        _plot_metric_axis(ax1, season_labels, possession_vals, _POSSESSION_STYLE)
        _plot_metric_axis(ax2, season_labels, shot_accuracy_vals, _SHOT_ACCURACY_STYLE)
